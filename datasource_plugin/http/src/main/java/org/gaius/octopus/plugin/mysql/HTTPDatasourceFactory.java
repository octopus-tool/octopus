 ...
 */
@Slf4j
public class HTTPDatasourceFactory implements DatasourceFactory<HTTPDatasourceInstance, Object> {
    
    // 每一个数据源ID对应一个数据源实例
    private static final Map<Long, HTTPDatasourceInstance> DATASOURCE_INSTANCE_MAP = new ConcurrentHashMap<>();
    
    // 对象锁
    private static final Object LOCK = new Object();
    
    @Override
    public HTTPDatasourceInstance create(DatasourceProperties config) {
        // 基于数据源配置，判断数据源是否需要进行连接池创建
        String tenantId = config.getTenantId();
        Long datasourceId = config.getDatasourceId();
        if (datasourceId == null) {
            return new HTTPDatasourceInstance(config);
        }
        ...
        synchronized (LOCK) {
            if (DATASOURCE_INSTANCE_MAP.containsKey(datasourceId)) {
                ...
                HTTPDatasourceInstance datasourceInstance = new HTTPDatasourceInstance(config);
    ...
    public void destroy(DatasourceProperties properties) {
        ...
        HTTPDatasourceInstance instance = DATASOURCE_INSTANCE_MAP.remove(datasourceId);
        if (instance == null) {
